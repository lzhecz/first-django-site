...
def about(request):
    return render(request, 'games/about.html', {'menu': menu, 'title': 'About'})


class AddGame(LoginRequiredMixin, DataMixin, CreateView):
    form_class = AddPostForm
    template_name = 'games/addgame.html'
...
#     else:
#         form = AddPostForm()
#     return render(request, 'games/addgame.html', {'form': form, 'menu': menu, 'title': 'Добавление игры'})
...
#     return HttpResponse(f"<h1>Games Calendar</h1><p>{year}</p>")
...
class GamesCategory(DataMixin, ListView):
    ...
    def get_queryset(self):
        return Games.objects.filter(cat__slug = self.kwargs['cat_slug'], is_published = True).select_related('cat')
